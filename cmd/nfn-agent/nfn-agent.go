...
func subscribeNotif(client pb.NfnNotifyClient, clusterclient kube.Interface, criclient criclient.CRIClient) error {
	...
	for {
		...
		for {
			...
			handleNotif(in, clusterclient, criclient)
		}
	}
}
...
func handleNotif(msg *pb.Notification, clusterclient kube.Interface, criclient criclient.CRIClient) {
...
func main() {
	...
	node, err := clustercli.GetNode(os.Getenv("HOSTNAME"))
	if err != nil {
	...
	go subscribeNotif(client, cniserver.GetClusterClient(), criclient)
