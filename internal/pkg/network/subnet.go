...
func ParseConfig(s string) (*Config, error) {
	...
	//fmt.Printf("cfg.SubnetLen=%v\n", cfg.SubnetLen)
	//fmt.Printf("cfg.Network.IP=%v\n", cfg.Network.IP)
	//fmt.Printf("cfg.Network.Next().IP=%v\n", cfg.Network.Next().IP)

	currentIP := cfg.Network.IP
	var subnets []string
	for i := 0; currentIP != cfg.Network.Next().IP; i++ {
		subnets = append(subnets, fmt.Sprintf("%s/%s", currentIP, strconv.FormatUint(uint64(cfg.SubnetLen), 10)))
		currentIP = currentIP + subnetSize
	}

	log.V(1).Info("Value of the subnets", "subnets", subnets)
	return cfg, nil
}

func calculateSubnet() {
	netConfPath := "/root/go/src/misc/subnet/net-conf.json"
	...
	_, err = ParseConfig(string(netConf))
	if err != nil {
		panic(err.Error())
	}
}
