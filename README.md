...
You will have to make some changes to Nodus deployment to make it compatibl with OpenShift. In file `deploy/ovn4nfv-k8s-plugin.yaml` please change definition of `cni` and `cnibin`  `volumes`  in `ovn4nfv-cni` DaemonSet to:
```
    - name: cni
      hostPath:
