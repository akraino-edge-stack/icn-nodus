...
func ParseConfig(s string) ([]v1alpha1.IpSubnet, error) {
	...
	//log.Info("Value of the virutal network slice", "currentIP", currentIP)
	//log.Info("Value of the virutal network slice", "cfg.Network.Next().IP", cfg.Network.Next().IP)
	...
	for i := 0; currentIP != cfg.Network.Next().IP; i++ {
		...
		//log.Info("Value of the virutal network slice", "_n.name", fmt.Sprintf("%s%s", virutalnetwork, strconv.Itoa(i)))
		//log.Info("Value of the virutal network slice", "_n.subnet", fmt.Sprintf("%s/%s", currentIP, strconv.FormatUint(uint64(cfg.SubnetLen), 10)))
		//log.Info("Value of the virutal network slice", "_n.gateway", fmt.Sprintf("%s", currentIP+ip.IP4(1)))
		//log.Info("Value of the virutal network slice", "_n.excludeIps", fmt.Sprintf("%s", currentIP+ip.IP4(1)))
		//log.Info("Value of the virutal network slice", "n.name", n.Name)
		//log.Info("Value of the virutal network slice", "n.subnet", n.Subnet)
		//log.Info("Value of the virutal network slice", "n.gateway", n.Gateway)
		//log.Info("Value of the virutal network slice", "n.excludeIps", n.ExcludeIps)
		vn = append(vn, n)
		//log.Info("Value of the virutal network slice", "vn", vn)
		currentIP = currentIP + subnetSize
		//log.Info("Value of the virutal network slice", "n", currentIP)
	}
	...
	//for j, net := range vn {
	//log.Info("Value of the virutal network slice", "j", j, "net.name", net.Name)
	//log.Info("Value of the virutal network slice", "j", j, "net.subnet", net.Subnet)
	//log.Info("Value of the virutal network slice", "j", j, "net.gateway", net.Gateway)
	//log.Info("Value of the virutal network slice", "j", j, "net.excludeIps", net.ExcludeIps)
	//}
