...
// PreparePortName prepares port name based on namespace and pod's name
func PreparePortName(parts... string) string {
	portName := ""
	for i, part := range parts {
		if i == 0 {
			portName = part
		} else {
			portName = fmt.Sprintf("%s_%s", portName, part)
		}
	}
	return fmt.Sprintf("%s_%s", portName, Hash(portName))
}
