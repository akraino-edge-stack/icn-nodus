...
func CreateVirutalNetwork() error {
	...
	for k, n := range nets {
		log.Info("Value of the virutal Networks", "k", k, "n.name", n.Name)
		log.Info("Value of the virutal Networks", "k", k, "n.name", n.Subnet)
		log.Info("Value of the virutal Networks", "k", k, "n.name", n.Gateway)
		log.Info("Value of the virutal Networks", "k", k, "n.name", n.ExcludeIps)
...
func CreateNetwork(id int, sn k8sv1alpha1.IpSubnet) error {
	...
	//resp, err = cli.Pods("default").Create(req)
