...
func createOvnLS(name, subnetv4, gatewayIPv4, excludeIps, subnetv6, gatewayIPv6 string) (gatewayIPv4Mask, gatewayIPv6Mask string, err error) {
	...
	if excludeIps != "" {
		stdout, stderr, err = RunOVNNbctl("--wait=hv", "--", "--may-exist", "ls-add", name, "--", "set", "logical_switch", name, otherConfig, externalIds, "other-config:exclude_ips="+excludeIps)
	} else {
		stdout, stderr, err = RunOVNNbctl("--wait=hv", "--", "--may-exist", "ls-add", name, "--", "set", "logical_switch", name, otherConfig, externalIds)
	}
	if err != nil {
...
func getOvnLSOtherConfig(ipNetv4, ipNetv6 *net.IPNet) string {
	config := ""
	if ipNetv6 != nil {
		config = "ipv6_prefix=" + ipNetv6.IP.String()
	}
	if ipNetv4 != nil {
		if config == "" {
			config = "subnet=" + ipNetv4.String()
		} else {
			config = config + ",subnet=" + ipNetv4.String()
		}
	}
	return "other-config:" + config
}

func getOvnLSExternalIds(gatewayIPv4Mask, gatewayIPv6Mask string) string {
	externalIds := ""
	if gatewayIPv6Mask != "" {
		externalIds = "gateway_ipv6=" + gatewayIPv6Mask
	}
	if gatewayIPv4Mask != "" {
		if externalIds == "" {
			externalIds = "gateway_ip=" + gatewayIPv4Mask
		} else {
			externalIds = externalIds + ",gateway_ip=" + gatewayIPv4Mask
		}
	}
	return "external-ids:" + externalIds
}
