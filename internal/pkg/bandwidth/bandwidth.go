...
func ParseBandwidthAsRate(bandwidth string) (string, error) {
	bandwidthQuantity, err := resource.ParseQuantity(bandwidth)
	if err != nil {
		return "", fmt.Errorf("parsing bandwidth %s failed: %s", bandwidth, err)
	}

	bandwidthInt, ok := bandwidthQuantity.AsInt64()
	if !ok {
		return "", fmt.Errorf("parsing bandwidth %s as Int64 failed: %s", bandwidth, err)
	}

	return strconv.FormatInt(bandwidthInt, 10), nil
}
