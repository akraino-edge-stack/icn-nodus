...
func NewCNIServer(serverRunDir string, k8sclient kubernetes.Interface) *CNIServer {
	...
	sec, err := kubecli.GetSecret(namespace, auth.DefaultCniCert)
	if sec == nil || err != nil{
		klog.Infof("Unable to obtain the secret. Will try to create new:", err)
		_, sec, err = auth.CreateCniCert(auth.DefaultCniCert, namespace, "dummy")
		if err != nil {
	...
	// err = auth.SaveCertfiles(sec, auth.CniCertPath)
	// if err != nil {
	// 	klog.Errorf("Error while saving certificate data from secret: %v", err)
	// 	return nil
	// }
